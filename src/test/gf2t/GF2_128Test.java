...
public class GF2_128Test {
    ...
    private static GF2_128 zero = new GF2_128(0);
    private static GF2_128 one = new GF2_128(1);
    ...
    public void constructorAndEqualityTest() {
        ...
        t = new GF2_128(b2, 6);
        s = t.toLongArray();
        assertFalse("Fail: constructor on byte array with offset", r[0] != s[0] || r[1] != s[1]);
        ...
        t = new GF2_128(b2, 6);
        s = t.toLongArray();
        assertFalse("Fail: constructor on byte array with offset of all 1s", r[0] != s[0] || r[1] != s[1]);
    ...
    public void pow2To2ToKTest() {
        ...
        for (long[] p : testValues) {
            for (int k=0; k<maxK; k ++) {
                z = new GF2_128(p);
                GF2_128.power2To2ToK(res, z, k);
                if (k==0) {
                    // Ground truth for squaring: self-multiply
                    GF2_128.mul(res1, z, z); // sqr should equal power2To2ToK with k = 0
                    assertFalse("Fail: power2To2To1  " + z, !res.equals(res1));
                    GF2_128.sqr(res2, z); // sqr should equal self-multiply with k = 0
                    ...
                    GF2_128.power2To2ToK(res1, res1,k-1);
                ...
                GF2_128.power2To2ToK(z, z, k); // power2To2ToK into same location
                assertFalse("Fail: power2To2ToK in place for k = " +k + " value = " + new GF2_128(p), !res.equals(z));
                if (k==0) {
                    z = new GF2_128(p);
                    GF2_128.sqr(z, z); // sqr into same location
                    assertFalse("Fail: sqr in place " + new GF2_128(p), !res.equals(z));
    ...
    public void interpolateTest() {
        ...
        GF2_128 [] optArray = new GF2_128[2];
        optArray[0] = null;
        optArray[1] = new GF2_128(17);
        ...
        GF2_128_Poly p = GF2_128_Poly.interpolate(new byte[0], new GF2_128[0], new GF2_128(0));
        ...
        GF2_128 val17 = new GF2_128(17);
        p = GF2_128_Poly.interpolate(new byte[0], new GF2_128[0], val17);
        ...
        for (int len = 1; len < 100; len++) {
            ...
            res = GF2_128_Poly.interpolate(points, values, null);
            for (int i = 0; i < len; i++) {
            ...
            res = GF2_128_Poly.interpolate(points, values, valueAt0);
            for (int i = 0; i < len; i++) {
                GF2_128 t = res.evaluate(points[i]);
            ...
            GF2_128_Poly t1 = GF2_128_Poly.fromByteArray(valueAt0.toByteArray(), b);
            ...
            assertFalse("To byte array round trip error at coeff0", !Arrays.equals(valueAt0.toByteArray(), Arrays.copyOfRange(b2, 0, 16)));
            assertFalse("To byte array round trip error with coeff0 at later coeff", !Arrays.equals(b1, Arrays.copyOfRange(b2, 16, b2.length)));
