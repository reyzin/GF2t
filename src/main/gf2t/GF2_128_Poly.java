...
public class GF2_128_Poly {
     ...
     * as a 16-byte representation of a GF2_128 value. Coefficient of degree 1 should
     * start at moreCoeffs[0].
     * @param coeff0 byte array representing lowest coefficient (16 bytes)
     * @param moreCoeffs byte array with concatenation of byte-converted coefficients
     *                   (16 bytes each) from degree 1 to the highest
     */
    public GF2_128_Poly (byte[] coeff0, byte[] moreCoeffs) {
        deg = moreCoeffs.length/16;
        c = new GF2_128[deg+1];
        c[0] = new GF2_128(coeff0);
        for (int i = 1; i<=deg; i++) {
            c[i] = new GF2_128(moreCoeffs, (i-1)*16);
        }
    }

    /**
     * Factory constructor -- same as GF2_128_Poly(coeff0, moreCoeffs)
     * @param coeff0 byte array representing lowest coefficient (16 bytes)
     * @param moreCoeffs byte array with concatenation of byte-converted coefficients
     *                   (16 bytes each) from degree 1 to the highest
     * @return new polynomial with the given coefficients
     */
    public static GF2_128_Poly fromByteArray(byte[] coeff0, byte[] moreCoeffs) {
        return new GF2_128_Poly(coeff0, moreCoeffs);
    }
    ...
    public  byte[] toByteArray(Boolean coeff0) {
        ...
        byte [] ret = new byte[(deg+1-c0)*16];
        for (int i=c0; i<=deg; i++) {
            c[i].toByteArray(ret, (i-c0)*16);
